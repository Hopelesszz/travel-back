...
export const getUserAwardsByUserAndAward = async (req, res, next) => {
  try {
    const { userId, awardId } = req.query;
    const userAward = await UserAward.findOne({ userId, achievementId: awardId });
    res.status(200).json(userAward);
  } catch (err) {
    next(err);
  }
